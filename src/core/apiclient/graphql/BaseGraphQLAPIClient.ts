import {DocumentNode, ApolloQueryResult, FetchResult, ApolloError} from "@apollo/client";
...
export class BaseGraphQLAPIClient {
  ...
  static async request<TRequest>(config: BaseGraphQLRequestConfig<TRequest, RequestMethod>): Promise<BaseGraphQLAPIResponse<TRequest, RequestMethod>> {
    ...
    const response = await (requestMethod === "query"
      ? client
          .query({
            context: {
              ...context,
          ...
          .then(async (rawResponse) => ({success: true as const, request, rawResponse}))
          .catch(async (rawResponse) => ({success: false as const, request, rawResponse}))
      : client
          .mutate({
            context: {
              ...context,
    ...
    const operationName = config.gql.definitions[0]?.kind === "OperationDefinition" ? config.gql.definitions[0].name?.value : "unknown";
    console.log(config.requestMethod.toUpperCase(), operationName, response, config);
